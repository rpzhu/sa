...
//把from的子对象都复制一份到to
void
copy_childs(const SAItem* from, SAItem* to);
//打印一个item内容
QDebug&
print_one_item(QDebug& dbg, const SAItem& item, const QString& prefix, bool isNewline = true);
QDebug&
print_item_and_child_items(QDebug& dbg, const SAItem& item, int indent);
void
copy_childs(const SAItem* from, SAItem* to)
{
...
SAItem::SAItem(SAItem* parentItem)
  : d_ptr(new SAItemPrivate(this))
{
    if (parentItem) {
...
SAItem::SAItem(const QString& text)
  : d_ptr(new SAItemPrivate(this))
{
    setName(text);
}

SAItem::SAItem(const QIcon& icon, const QString& text)
  : d_ptr(new SAItemPrivate(this))
{
    setName(text);
    setIcon(icon);
...
///
void
SAItem::setName(const QString& name)
{
    d_ptr->m_name = name;
}
///
...
///
void
SAItem::setIcon(const QIcon& icon)
{
...
///
void
SAItem::setProperty(int roleID, const QVariant& var)
{
    if (RoleName == roleID) {
...
///
bool
SAItem::isHaveProperty(int roleID) const
{
    if (RoleName == roleID) {
        return true;
    }
...
///
void
SAItem::property(int index, int& id, QVariant& var) const
{
 ...
 */
QVariant
SAItem::getProperty(int id, const QVariant& defaultvar) const
{
...
///
void
SAItem::appendChild(SAItem* item)
{
...
///
void
SAItem::insertChild(SAItem* item, int row)
{
 ...
 */
bool
SAItem::haveChild(SAItem* const item) const
{
 ...
 */
bool
SAItem::takeChild(SAItem* const item)
{
 ...
 */
int
SAItem::childIndex(SAItem* const item) const
{
 ...
 */
void
SAItem::removeChild(SAItem* item)
{
    int index = childIndex(item);
    if (index >= 0) {
...
void
SAItem::_setTreePtr(SATree* tree)
{
    d_ptr->m_tree = tree;
}
 ...
 */
QDebug&
operator<<(QDebug& dbg, const SAItem& item)
{
...
QDebug&
print_one_item(QDebug& dbg, const SAItem& item, const QString& prefix, bool isNewline)
{
