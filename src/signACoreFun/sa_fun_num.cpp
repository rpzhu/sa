...
///
std::shared_ptr<SAVariantDatas>
saFun::mean(SAAbstractDatas* data)
{
    QVector<double> vd;
    if (!SADataConver::converToDoubleVector(data, vd)) {
        setErrorString(TR("data can not conver to double vector"));
        return SAValueManager::makeData<SAVariantDatas>(false);
    }
...
///
double
saFun::meamPointY(const QVector<QPointF>& points)
{
    if (points.size() > 0)
...
///
std::shared_ptr<SAVariantDatas>
saFun::sum(SAAbstractDatas* data)
{
    QVector<double> vd;
    if (!SADataConver::converToDoubleVector(data, vd)) {
        setErrorString(TR("data can not conver to double vector"));
...
///
double
saFun::sumPointY(const QVector<QPointF>& points)
{
    double res = 0.0;
...
saFun::add(SAAbstractDatas* a, SAAbstractDatas* b)
{
    return double_transform(a, b, [](const double& a, const double& b) -> double { return a + b; });
}
...
///
std::shared_ptr<SAAbstractDatas>
saFun::subtract(SAAbstractDatas* a, SAAbstractDatas* b)
{
    return double_transform(a, b, [](const double& a, const double& b) -> double { return a - b; });
}
...
///
std::shared_ptr<SAAbstractDatas>
saFun::multiplication(SAAbstractDatas* a, SAAbstractDatas* b)
{
    return double_transform(a, b, [](const double& a, const double& b) -> double { return a * b; });
}
...
///
std::shared_ptr<SAAbstractDatas>
saFun::division(SAAbstractDatas* a, SAAbstractDatas* b)
{
    return double_transform(a, b, [](const double& a, const double& b) -> double { return a / b; });
}
...
/// \return 计算的结果
///
std::shared_ptr<SAVectorDouble>
saFun::diff(SAAbstractDatas* data, unsigned diffCount)
{
...
///
std::shared_ptr<SATableVariant>
saFun::statistics(SAAbstractDatas* data)
{
    QVector<double> vd;
    ...
    int r = 0;
    varTable->setTableData(r, 0, TR("sum"));
    varTable->setTableData(r++, 1, sum);
    varTable->setTableData(r, 0, TR("mean"));
    varTable->setTableData(r++, 1, mean);
    varTable->setTableData(r, 0, TR("var"));
    varTable->setTableData(r++, 1, var);
    varTable->setTableData(r, 0, TR("std"));
    varTable->setTableData(r++, 1, std);
    varTable->setTableData(r, 0, TR("skewness"));
    varTable->setTableData(r++, 1, skewness);
    varTable->setTableData(r, 0, TR("kurtosis"));
    varTable->setTableData(r++, 1, kurtosis);
    varTable->setTableData(r, 0, TR("max"));
    varTable->setTableData(r++, 1, *minmax.second);
    varTable->setTableData(r, 0, TR("min"));
    varTable->setTableData(r++, 1, *minmax.first);
    varTable->setTableData(r, 0, TR("peak to peak value"));
    varTable->setTableData(r++, 1, ppv);
    return varTable;
}

std::shared_ptr<SAVectorInterval>
saFun::hist(const SAAbstractDatas* wave, unsigned section)
{
    QVector<double> ys;
    ...
    SA::count_frequency(ys.begin(), ys.end(), section, sectionRang.begin(), freCount.begin());
...
QMap<QString, double>
saFun::statistics(const QVector<double>& data)
{
