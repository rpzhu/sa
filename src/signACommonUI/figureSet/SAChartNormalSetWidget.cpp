...
public:
    ...
    void setupUI(SAChartNormalSetWidget* par)
    {
        ...
        //Title
        titleEdit = new SALineEditPropertyItem(); //类似lineedit的输入框
        titleEdit->setObjectName("titleEdit");
        par->connect(titleEdit, &SALineEditPropertyItem::textChanged, par, &SAChartNormalSetWidget::onTitleTextChanged);
        //Footer
        footerEdit = new SALineEditPropertyItem();
        footerEdit->setObjectName("footerEdit");
        par->connect(footerEdit, &SALineEditPropertyItem::textChanged, par, &SAChartNormalSetWidget::onFooterTextChanged);
        //Canvas Background
        canvasBackgroundEdit = new SAColorSetPropertyItem();
        par->connect(canvasBackgroundEdit, &SAColorSetPropertyItem::colorChanged, par, &SAChartNormalSetWidget::onCanvasBackgroundColorChanged);
        //borderRadius
        borderRadiusEdit = new SADoubleSpinBoxPropertyItem();
        par->connect(borderRadiusEdit, &SADoubleSpinBoxPropertyItem::valueChanged, par, &SAChartNormalSetWidget::onBorderRadiusChanged);
...
SAChartNormalSetWidget::SAChartNormalSetWidget(QWidget* par)
  : QWidget(par)
...
void
SAChartNormalSetWidget::setChart(SAChart2D* chart)
{
    ui->setChart(chart);
...
void
SAChartNormalSetWidget::retranslateUi()
{
    ui->retranslateUi(this);
}

void
SAChartNormalSetWidget::updateData()
{
    ui->updateData();
}

void
SAChartNormalSetWidget::paintEvent(QPaintEvent* e)
{
    QWidget::paintEvent(e);
}

void
SAChartNormalSetWidget::onTitleTextChanged(const QString& text)
{
    if (ui->chart) {
...
void
SAChartNormalSetWidget::onFooterTextChanged(const QString& text)
{
    if (ui->chart) {
...
void
SAChartNormalSetWidget::onCanvasBackgroundColorChanged(const QColor& clr)
{
    if (ui->chart) {
...
void
SAChartNormalSetWidget::onBorderRadiusChanged(double v)
{
    if (ui->chart) {
