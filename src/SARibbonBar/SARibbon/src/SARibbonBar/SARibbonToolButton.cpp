...
QDebug
operator<<(QDebug debug, const QStyleOptionToolButton& opt)
{
    debug << "=============="
          << "\nQStyleOption("
          << (QStyleOption)opt
...
void
SARibbonToolButton::paintEvent(QPaintEvent* event)
{
    switch (m_buttonType) {
    case LargeButton:
...
void
SARibbonToolButton::resizeEvent(QResizeEvent* e)
{
...
void
SARibbonToolButton::mouseMoveEvent(QMouseEvent* e)
{
...
void
SARibbonToolButton::mousePressEvent(QMouseEvent* e)
{
...
void
SARibbonToolButton::mouseReleaseEvent(QMouseEvent* e)
{
...
void
SARibbonToolButton::focusOutEvent(QFocusEvent* e)
{
...
void
SARibbonToolButton::leaveEvent(QEvent* e)
{
    m_mouseOnSubControl = false;
...
void
SARibbonToolButton::paintSmallButton(QPaintEvent* e)
{
    Q_UNUSED(e);
...
void
SARibbonToolButton::paintLargeButton(QPaintEvent* e)
{
    Q_UNUSED(e);
...
bool
SARibbonToolButton::hitButton(const QPoint& pos) const
{
...
void
SARibbonToolButton::drawIconAndLabel(QPainter& p, const QStyleOptionToolButton& opt)
{
    if (LargeButton == m_buttonType) {
        //绘制图标和文字
        ...
        if (((!hasArrow && opt.icon.isNull()) && !opt.text.isEmpty()) || opt.toolButtonStyle == Qt::ToolButtonTextOnly) { //没有箭头 且 没图标 有文字 || 只有文字模式
            ...
            if (!opt.icon.isNull()) { //有图标
                ...
                if (!(opt.state & QStyle::State_Enabled))
                    ...
                    mode = QIcon::Normal;
                pm = opt.icon.pixmap(this->window()->windowHandle(), opt.rect.size().boundedTo(opt.iconSize), mode, state);
            ...
            if (opt.toolButtonStyle != Qt::ToolButtonIconOnly) {
                ...
                style()->drawItemText(&p, QStyle::visualRect(opt.direction, rect, tr), alignment, opt.palette, opt.state & QStyle::State_Enabled, opt.text, QPalette::ButtonText);
            } else {
                rect.translate(shiftX, shiftY);
                if (hasArrow) {
        ...
        if (!opt.icon.isNull()) {
            ...
            if (opt.toolButtonStyle != Qt::ToolButtonIconOnly) {
                ...
                style()->drawItemText(&p, QStyle::visualRect(opt.direction, opt.rect, tr), alignment, opt.palette, opt.state & QStyle::State_Enabled, opt.text, QPalette::ButtonText);
                p.restore();
            } else {
        ...
        } else // 只有文字
        {
            ...
            style()->drawItemText(&p, QStyle::visualRect(opt.direction, opt.rect, opt.rect.adjusted(2, 1, -2, -1)), alignment, opt.palette, opt.state & QStyle::State_Enabled, opt.text, QPalette::ButtonText);
        }
...
void
SARibbonToolButton::drawArrow(const QStyle* style, const QStyleOptionToolButton* toolbutton, const QRect& rect, QPainter* painter, const QWidget* widget)
{
...
SARibbonToolButton::RibbonButtonType
SARibbonToolButton::buttonType() const
{
    return m_buttonType;
}

void
SARibbonToolButton::setButtonType(const RibbonButtonType& buttonType)
{
...
QSize
SARibbonToolButton::minimumSizeHint() const
{
    return sizeHint();
}

bool
SARibbonToolButton::event(QEvent* e)
{
    switch (e->type()) {
