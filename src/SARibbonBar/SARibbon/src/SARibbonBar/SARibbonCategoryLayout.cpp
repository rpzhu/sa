...
SARibbonCategory*
SARibbonCategoryLayout::ribbonCategory()
{
    return qobject_cast<SARibbonCategory*>(parent());
}

void
SARibbonCategoryLayout::addItem(QLayoutItem* item)
{
...
QLayoutItem*
SARibbonCategoryLayout::itemAt(int index) const
{
    return itemList.value(index, Q_NULLPTR);
}

QLayoutItem*
SARibbonCategoryLayout::takeAt(int index)
{
...
int
SARibbonCategoryLayout::count() const
{
    return itemList.size();
}

QSize
SARibbonCategoryLayout::sizeHint() const
{
    QSize size;
    // QLayoutItem *item;
...
QSize
SARibbonCategoryLayout::minimumSize() const
{
    return QSize(50, 10);
}

Qt::Orientations
SARibbonCategoryLayout::expandingDirections() const
{
    return Qt::Horizontal;
}

void
SARibbonCategoryLayout::invalidate()
{
    m_isChanged = true;
    QLayout::invalidate();
}
...
int
SARibbonCategoryLayout::buildReduceModePannel(SARibbonPannel* realPannel, int x, int y)
{
    ...
    SARibbonToolButton* btn = reducePannel->addLargeToolButton(realPannel->windowTitle(), realPannel->windowIcon(), QToolButton::InstantPopup);
    info.reduceModeShowPannel = reducePannel;
    reducePannel->move(x, y);
    connect(btn, &SARibbonToolButton::clicked, this, [categoryPage, info](bool on) {
        Q_UNUSED(on);
...
QPoint
SARibbonCategoryLayout::calcPopupPannelPosition(SARibbonCategory* category, SARibbonPannel* pannel, int x)
{
...
void
SARibbonCategoryLayout::setGeometry(const QRect& rect)
{
    if (rect == geometry()) {
        qWarning() << "warning !!!!rect == geometry() at SARibbonCategoryLayout::setGeometry(const QRect &rect) ";
        return;
    }
    ...
    for (int i = 0; i < listSize; ++i) {
        ...
        if (nextX > rect.right()) {
            //说明超出边界
            if (SARibbonPannel* pannel = qobject_cast<SARibbonPannel*>(wid)) { //此时无法显示全一个pannel
